...
export default function VehicleSpeedGovernorEditModal({ show, onHide, record, onUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post(`/speed-governors/${record.id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      toast.success('Speed Governor record updated.');
      onUpdated?.(); onHide();
    } catch (err) {
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      <Form onSubmit={handleSubmit}>
        <Modal.Header closeButton><Modal.Title>Edit Speed Governor Record</Modal.Title></Modal.Header>
        <Modal.Body>
